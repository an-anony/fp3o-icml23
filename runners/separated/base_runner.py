...
class Runner(object):
    ...
    def train(self):
        ...
        for stage, agent_pair in enumerate(agents_pairs):
            parallel_trainer = PPO_Trainer(self.all_args, [self.trainer[agent_id] for agent_id in agent_pair])
                ...
                for agent_id in agent_pair:
                    ...
                    with torch.no_grad():
                        if self.use_recurrent_policy:
                            ...
                            old_actions_logprob, _ =self.trainer[agent_id].policy.actor.evaluate_actions(*input)
                    ...
                    # old_actions_logprob_pair.append(old_actions_logprob)
            ...
            if stage == 0:
                for agent_id in agent_pair:
                    ...
                    with torch.no_grad():
                        if self.use_recurrent_policy:
                            input = self.buffer[agent_id].data_chunk(self.data_chunk_length)
                            new_actions_logprob, _ =self.trainer[agent_id].policy.actor.evaluate_actions(*input)
