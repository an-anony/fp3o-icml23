...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space):
        ...
        self.factor = None
        self.factor_j_p = None

        self.step = 0

    def data_chunk(self, data_chunk_length):
        episode_length, n_rollout_threads = self.rewards.shape[0:2]
        batch_size = n_rollout_threads * episode_length
        data_chunks = batch_size // data_chunk_length  # [C=r*T/L]
        indices = np.arange(data_chunks)
        ...
        L, N = data_chunk_length, data_chunks
        ...
        return obs_batch, rnn_states_batch, actions_batch, masks_batch, available_actions_batch, active_masks_batch
        

    def reverse_data_chunk(self, actions_logprob, data_chunk_length):
        def _t2n(x):
            return x.detach().cpu().numpy()

        tpdv = dict(dtype=torch.float32, device=actions_logprob.device)
        actions_logprob = _t2n(actions_logprob)
        ...
        # actions_logprob = actions_logprob.reshape(L, N, *actions_logprob.shape[1:])
        # actions_logprob = actions_logprob.transpose(1, 0, 2).reshape(-1, *actions_logprob.shape[2:])
        # actions_logprob = actions_logprob.reshape(n_rollout_threads, episode_length, *actions_logprob.shape[1:]).transpose(1, 0, 2)
        # actions_logprob = actions_logprob.reshape(-1, *actions_logprob.shape[2:])

        actions_logprob = actions_logprob.reshape(L, N, *actions_logprob.shape[1:]).transpose(1, 0, 2)
        actions_logprob = actions_logprob.reshape(n_rollout_threads, episode_length, *actions_logprob.shape[2:]).transpose(1, 0, 2)
        actions_logprob = actions_logprob.reshape(-1, *actions_logprob.shape[2:])

        return check(actions_logprob).to(**tpdv)


    def update_factor(self, factor):
        self.factor = factor.copy()

    def update_factor_j_p(self, factor_j_p):
        self.factor_j_p = factor_j_p.copy()
    ...
    def subject_to_generator(self, advantages):

        factor_batch = self.factor.reshape(-1, self.factor.shape[-1])
        factor_j_p_batch = self.factor_j_p.reshape(-1, self.factor_j_p.shape[-1])
        adv_targ = advantages.reshape(-1, 1)
        active_masks_batch = self.active_masks[:-1].reshape(-1, 1)

        yield adv_targ, active_masks_batch, factor_batch, factor_j_p_batch
